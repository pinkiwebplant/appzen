/******/ (() => {
      ...
      if (ScrollTrigger_gsap) {
        ...
        if (ScrollTrigger_body) {
          ...
          ScrollTrigger_addListener(
            ...
            "touchstart",
            ScrollTrigger_passThrough
          ); //works around Safari bug: https://gsap.com/forums/topic/21450-draggable-in-iframe-on-mobile-is-buggy/
  ...
  ScrollTrigger_getGSAP() &&
    ScrollTrigger_gsap.registerPlugin(ScrollTrigger_ScrollTrigger); // CONCATENATED MODULE: ./src/assets/js/sections/floating-content-media.js
  ...
  function floating_content_media_defineProperties(e, r) {
  ...
  function floating_content_media_createClass(e, r, t) {
    return (
      r && floating_content_media_defineProperties(e.prototype, r),
      t && floating_content_media_defineProperties(e, t),
  ...
  var FloatingContentMedia = /*#__PURE__*/ (function () {
    function FloatingContentMedia(section) {
      _classCallCheck(this, FloatingContentMedia);
      this.section = section;
      this.cards = this.section.find(".floating-media-card");
      this.cardsLength = this.cards.length;
      this.hH = $("header").outerHeight();
      this.isClicked = false;
    }
    return floating_content_media_createClass(FloatingContentMedia, [
      {
        key: "calcHeights",
        value: function calcHeights() {
          var _this = this;
          var maxHeight = Math.max.apply(
            null,
            this.cards
              .map(function (ind) {
                return _this.cards.eq(ind).outerHeight();
              })
              .get()
          );
          this.cards.css({
          ...
          var stickyCardsSectionHeight = this.section.outerHeight();
          this.section.attr("data-height", stickyCardsSectionHeight);
          this.appendDots(maxHeight);
        },
      },
      {
        key: "appendDots",
        value: function appendDots(mH) {
          var _this2 = this;
          this.cards
            .eq(0)
            .prepend(
              '<div class="floating-media-card-dots" style="top:'.concat(
                mH / 2,
                'px"></div>'
              )
            );
          this.cards.each(function (id) {
            var cardId = _this2.cards.eq(id).attr("id");
            var isActive = id == 0 ? " active" : "";
            _this2.cards
              .eq(0)
              .find(".floating-media-card-dots")
              .append(
                '<a class="floating-media-card-dot'
                  .concat(isActive, '" id="dot-')
                  .concat(cardId, '" href="#')
                  .concat(cardId, '"></a>')
              );
          });
      ...
      {
        key: "setActiveDot",
        value: function setActiveDot(dot, isClicked) {
          if (!isClicked) {
      ...
      {
        key: "dotClickFunc",
        value: function dotClickFunc() {
          var _this3 = this;
          this.cards
            .eq(0)
            .find(".floating-media-card-dot")
            .each(function (id, dot) {
              var sec = $($(dot).attr("href"));
              var startPosition = id * 40 + 40 + _this3.hH;
              var linkSec = ScrollTrigger_ScrollTrigger.create({
                trigger: sec,
                start: "top ".concat(startPosition),
              });
              $(dot).on("click", function (e) {
                e.preventDefault();
                _this3.isClicked = true;
                $(dot).siblings().removeClass("active");
                $(dot).addClass("active");
                $("html, body").animate(
                  {
                    scrollTop: linkSec.start + 1,
                  },
                  1000,
                  function () {
                    setTimeout(function () {
                      _this3.isClicked = false;
                    }, 100);
                  }
                );
              });
      ...
      {
        key: "stickyCardsFunc",
        value: function stickyCardsFunc() {
          var _this4 = this;
          if ($(window).width() < 768 || this.cardsLength === 0) {
            return;
          }
          this.calcHeights();
          var imgClass = "floating-media-card-image";
          var contentClass = "floating-media-card-content";
          var cardClass = "floating-media-card";
          var stickyCardsImageGsap = gsapWithCSS.utils.toArray(
            this.section.find(".".concat(cardClass, " .").concat(imgClass))
          );
          var stickyCardsContentGsap = gsapWithCSS.utils.toArray(
            this.section.find(
              ".".concat(cardClass, ":not(:last-child) .").concat(contentClass)
            )
          );
          var marginFromHeader = 40;
          var delayTime = 100;
          stickyCardsImageGsap.forEach(function (image, index) {
            _this4.animateCardImage(
              image,
              index,
              stickyCardsImageGsap.length,
              marginFromHeader,
              delayTime
            );
          });
          stickyCardsContentGsap.forEach(function (content, index) {
            _this4.animateCardContent(
              content,
              index,
              marginFromHeader,
              delayTime
      ...
      {
        key: "animateCardImage",
        value: function animateCardImage(
          image,
          index,
          totalCards,
          marginFromHeader,
          ...
          var opacityDifference = 0.8;
          var tempStartPosition = index * 40 + marginFromHeader + this.hH;
          var startPosition =
            index === totalCards - 1 ? 160 : tempStartPosition - delayTime;
          var scaleValue = scaleDifference * (this.cardsLength - (index + 1));
          var opacityValue =
            1 -
            opacityDifference * ((this.cardsLength - index) / this.cardsLength);
          opacityValue = index === this.cardsLength - 1 ? 1 : opacityValue;
          var cardHeight =
            index === totalCards - 1 ? 0 : $(image).outerHeight() + delayTime;
          gsapWithCSS.to(image, {
            ...
            scrollTrigger: {
              trigger: image,
              start: "top ".concat(startPosition),
              end: "+=".concat(cardHeight),
              scrub: 0.8,
              invalidateOnResize: true,
              onLeave: function onLeave() {
                return _this5.onLeaveCardImage(index, totalCards);
              },
              onEnterBack: function onEnterBack() {
                return _this5.onEnterBackCardImage(index, totalCards, image);
              },
              onEnter: function onEnter() {
                return _this5.onEnterCardImage(image);
              },
            },
          });
        },
      ...
      {
        key: "animateCardContent",
        value: function animateCardContent(
          content,
          index,
          marginFromHeader,
          delayTime
          ...
          var scaleDifference = 0.07;
          var startPosition =
            index * 40 + marginFromHeader + this.hH - delayTime;
          var scaleValue = scaleDifference * (this.cardsLength - (index + 1));
          var cardHeight = this.cards.eq(index).outerHeight() + delayTime;
      ...
      {
        key: "onLeaveCardImage",
        value: function onLeaveCardImage(index, totalCards) {
          if (index === totalCards - 1) {
            this.section
              .addClass("remove-sticky")
              .css("height", this.section.data("height"));
          }
        },
      },
      {
        key: "onEnterBackCardImage",
        value: function onEnterBackCardImage(index, totalCards, image) {
          if (index === totalCards - 1) {
            this.section.removeClass("remove-sticky").height("auto");
          }
          if (index < totalCards - 1) {
            var currId = $(image).parent().attr("id");
            this.setActiveDot(
              $(".floating-media-card-dot#dot-".concat(currId)),
              this.isClicked
            );
          }
        },
      ...
      {
        key: "onEnterCardImage",
        value: function onEnterCardImage(image) {
          var currId = $(image).parent().attr("id");
          this.setActiveDot(
            $(".floating-media-card-dot#dot-".concat(currId)),
            this.isClicked
  ...
  $(function () {
    $(window).on("load", function () {
      var stickyCardsSection = $(".floating-media-cards");
      stickyCardsSection.each(function (id, section) {
        var sectionEl = $(section);
        var floatingContent = new FloatingContentMedia(sectionEl);
        floatingContent.init();
      });
    });
  });
  /******/
